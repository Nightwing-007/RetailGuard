...
Phase 4: feature engineering, train/test split, SMOTE oversampling, scaling, and persist scaler.
"""
...
def main():
    """Execute the feature engineering phase."""
    ...
    train_df, test_df = train_test_split(
        df, test_size=0.2, stratify=df["IsReturned"], random_state=RANDOM_STATE
    )
    ...
    cust_defaults = {
        "Customer_Total_Orders": (
            int(customer_agg["Customer_Total_Orders"].median())
            if not customer_agg.empty
            else 0
        ),
        "Customer_Total_Items": (
            float(customer_agg["Customer_Total_Items"].median())
            if not customer_agg.empty
            else 0.0
        ),
        "Customer_Average_Spend": (
            float(customer_agg["Customer_Average_Spend"].median())
            if not customer_agg.empty
            else 0.0
        ),
        "Customer_Return_Rate": (
            float(customer_agg["Customer_Return_Rate"].median())
            if not customer_agg.empty
            else 0.0
        ),
    }
    prod_defaults = {
        "Product_Total_Sales": (
            float(product_agg["Product_Total_Sales"].median())
            if not product_agg.empty
            else 0.0
        ),
        "Product_Total_Orders": (
            int(product_agg["Product_Total_Orders"].median())
            if not product_agg.empty
            else 0
        ),
        "Product_Return_Rate": (
            float(product_agg["Product_Return_Rate"].median())
            if not product_agg.empty
            else 0.0
        ),
    }
    ...
    X_train_scaled = pd.DataFrame(
        scaler.transform(X_resampled), columns=FEATURE_COLUMNS
    )
    X_test_scaled = pd.DataFrame(
        scaler.transform(X_test_imputed), columns=FEATURE_COLUMNS
    )
    ...
    pd.Series(y_resampled, name="IsReturned").to_csv(
        OUT_DIR / "y_train_resampled.csv", index=False
    )
